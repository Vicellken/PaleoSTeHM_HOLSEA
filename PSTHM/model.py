...
class GPRegression_EIV(GPModel):
    ...
    def __init__(
        self, X, y, x_noise, kernel, noise=None, mean_function=None, jitter=1e-6
    ):
        assert isinstance(
            X, torch.Tensor
        ), "X needs to be a torch Tensor instead of a {}".format(type(X))
    ...
    def model(self):
        ...
        N = self.X.size(0)
        x_noise = pyro.sample(
            "xerr", dist.Normal(torch.zeros(N), self.x_noise**0.5).to_event(1)
        )
        if self.X.dim() <= 1:
            X_noisy = self.X
...
def change_point_model(
    X, y, x_sigma, y_sigma, n_cp, intercept_prior, coefficient_prior
):
    """
    ...
    for i in range(n_cp + 1):
        ...
        mean[start_idx:end_idx] = (
            beta_coef_list[i] * (x_noisy[start_idx:end_idx] - last_change_point)
            + last_intercept
        )
        last_intercept = (
...
def ensemble_GIA_model(X, y, x_sigma, y_sigma, model_ensemble, model_age):
    ...
    weight_facor_list = pyro.sample(
        "W", dist.Dirichlet(torch.ones(model_ensemble.shape[0]))
    )
    ...
    for i in range(model_ensemble.shape[0]):
        GIA_model = interpolate.interp1d(
            model_age.detach().numpy(), model_ensemble[i].detach().numpy()
        )
...
def linear_model(
    X, y, x_sigma, y_sigma, intercept_prior, coefficient_prior, whitenoise_prior=None
):
    """
    A function to define a linear model in pyro
...
def linear_model_uniform(
    X, y, x_sigma, y_range, intercept_prior, coefficient_prior, whitenoise_prior
):
    """
