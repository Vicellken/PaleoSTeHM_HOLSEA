...
def SVI_optm(
    gpr, num_iteration=500, lr=0.1, decay_r=1, step_size=100, equal_kernels=None
):
    """
    ...
    scheduler = torch.optim.lr_scheduler.StepLR(
        optimizer, step_size=step_size, gamma=decay_r
    )
        ...
        if equal_kernels == None:
            ...
            for kernels in equal_kernels:
                ...
                for i5 in range(1, len(kernels)):
                    print(i5, kernels[i5])
                    pyro.get_param_store()[kernels[i5]] = pyro.get_param_store()[
                        kernels[0]
                    ]
...
def SVI_NI_optm(
    ...
    N = len(gpr.X)
    if x_sigma is None:
        x_sigma = torch.zeros(N, dtype=gpr.X.dtype, device=gpr.X.device)
    else:
        if x_sigma.numel() != N:
            raise ValueError(
                f"x_sigma must have {N} elements to match gpr.X, got {x_sigma.numel()}"
            )
    if gpr.noise.dim() == 1:
    ...
    for i in tqdm(range(num_iteration)):
        ...
        if y_rate.ndim > 1:
            y_rate = y_rate[:, 0]
        if gpu:
            x_sigma = x_sigma.to(x_test.device)
...
def NUTS_mcmc(
    gpr, num_samples=2500, warmup_steps=200, target_accept_prob=0.8, print_stats=False
):
    """
    ...
    if print_stats:
        for name, value in mcmc.get_samples().items():
            if "kernel" in name:
                ...
                print(
                    "-----{}: {:4.2f} +/ {:4.2f} (2sd)-----".format(
                        name, value.mean(), 2 * value.std()
                    )
                )
                print(
                    "Gelman-Rubin statistic for {}: {:4.2f}".format(
                        name, mcmc.diagnostics()[name]["r_hat"].item()
                    )
                )
                print(
                    "Effective sample size for {}: {:4.2f}".format(
                        name, mcmc.diagnostics()[name]["n_eff"].item()
                    )
                )
...
def opti_pyro_model(
    ...
    *args: additional arguments for the model
    ...
    scheduler = pyro.optim.ExponentialLR(
        {"optimizer": pyro.optim.Adam, "optim_args": {"lr": lr}, "gamma": decay_r}
    )
