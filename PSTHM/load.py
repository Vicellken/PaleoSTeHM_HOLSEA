...
def load_local_rsl_data(file):
    ...
    rsl = data["RSL"]
    rsl_2sd = (
        data["RSLer_up_2sd"] + data["RSLer_low_2sd"]
    ) / 2  # average up and low 2std
    rsl_age = -(data["Age"] - 1950)  # convert age from BP to CE
    rsl_age_2sd = (
        data["Age_low_er_2sd"] + data["Age_up_er_2sd"]
    ) / 2  # average up and low 2std
...
def load_regional_rsl_data(file, CE=False):
    ...
    marine_index, SLIP_index, terrestrial_index = (
        rsl_limiting == -1,
        rsl_limiting == 0,
        rsl_limiting == 1,
    )
    ...
    marine_limiting = [
        X[marine_index],
        y[marine_index],
        y_sigma[marine_index],
        x_sigma[marine_index],
        rsl_lon[marine_index].values,
        rsl_lat[marine_index].values,
        rsl_region[marine_index].values,
        rsl_region_name[marine_index].values,
    ]
    ...
    SLIP = [
        ...
        y_sigma[SLIP_index],
        x_sigma[SLIP_index],
        rsl_lon[SLIP_index].values,
        rsl_lat[SLIP_index].values,
        rsl_region[SLIP_index].values,
        rsl_region_name[SLIP_index].values,
    ]

    marine_limiting = [
        X[terrestrial_index],
        y[terrestrial_index],
        y_sigma[terrestrial_index],
        x_sigma[terrestrial_index],
        rsl_lon[terrestrial_index].values,
        rsl_lat[terrestrial_index].values,
        rsl_region[terrestrial_index].values,
        rsl_region_name[terrestrial_index].values,
    ]
...
def load_PSMSL_data(
    ...
    site_file = pd.read_table(
        data_folder + "/filelist.txt",
        delimiter=";",
        header=None,
    )
    US_AT_index = (
        (site_file.iloc[:, 1] >= min_lat)
        & (site_file.iloc[:, 1] <= max_lat)
        & (site_file.iloc[:, 2] >= min_lon)
        & (site_file.iloc[:, 2] <= max_lon)
    )
    ...
    for i, p in enumerate(US_AT_site):
        if US_AT_data is None:
            US_AT_data = pd.read_table(
                data_folder + str(p) + ".rlrdata", delimiter=";", header=None
            )
    ...
    for i in range(len(US_site_coord)):
        ...
        if (
            np.max(US_AT_data[site_index].iloc[:, 0])
            - np.min(US_AT_data[site_index].iloc[:, 0])
            >= min_time_span
        ):
        ...
        new_rsl[site_index] = (
            US_AT_data[site_index].iloc[:, 1] - US_AT_data[site_index].iloc[-1, 1]
        )
    US_AT_data.iloc[:, 1] = new_rsl
    data_filter_all = (
        data_filter & data_filter_2 & data_filter_3 & data_filter_4 & data_filter_5
    )
