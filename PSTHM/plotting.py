...
def plot_uncertainty_boxes(
    ...
    for i in range(len(x)):
        if (x_error.ndim == 1) and (y_error.ndim == 1):
            ...
            ax.add_patch(
                plt.Rectangle(
                    (x[i] - x_error[i][0], y[i] - y_error[i][0]),
                    x_error[i][1] + x_error[i][0],
                    y_error[i][1] + y_error[i][0],
                    fill=True,
                    fc=fc,
                    ec=ec,
                    linewidth=3,
    ...
    ax.add_patch(
        plt.Rectangle((0, 0), 0, 0, fill=True, fc=fc, ec=ec, linewidth=3, label="SLIPs")
    )
        ...
        if y_error.ndim == 1:
            if x_error.ndim == 1:
                ax.errorbar(
                    ...
                    y,
                    xerr=x_error,
                    yerr=[y_error, [0] * len(y_error)],
                    fmt="None",
                    color="steelblue",
                    label="Marine limiting",
                    linewidth=4,
                )
            elif x_error.ndim == 2:
                ax.errorbar(
        ...
        if y_error.ndim == 1:
            if x_error.ndim == 1:
                plt.errorbar(
                    ...
                    y,
                    xerr=x_error,
                    yerr=[[0] * len(y_error), y_error],
                    fmt="None",
                    color="green",
                    label="Terrestrial limiting",
                    linewidth=4,
                )
            elif x_error.ndim == 2:
                plt.errorbar(
...
def plot_tem_regression(
    ...
    if torch.is_tensor(data_age) == True:
        data_age = data_age.detach().numpy()
    if torch.is_tensor(data_rsl) == True:
        data_rsl = data_rsl.detach().numpy()
    if torch.is_tensor(data_age_sigma) == True:
        data_age_sigma = data_age_sigma.detach().numpy()
    if torch.is_tensor(data_rsl_sigma) == True:
        data_rsl_sigma = data_rsl_sigma.detach().numpy()
    if torch.is_tensor(mean_rsl_age) == True:
        mean_rsl_age = mean_rsl_age.detach().numpy()
    if torch.is_tensor(mean_rsl) == True:
        mean_rsl = mean_rsl.detach().numpy()
    if torch.is_tensor(rsl_sd) == True:
        rsl_sd = rsl_sd.detach().numpy()
    if torch.is_tensor(rsl_rate_age) == True:
        rsl_rate_age = rsl_rate_age.detach().numpy()
    if torch.is_tensor(mean_rate) == True:
        mean_rate = mean_rate.detach().numpy()
    if torch.is_tensor(rate_sd) == True:
        rate_sd = rate_sd.detach().numpy()

    if axes == None:
        fig, axes = plt.subplots(nrows=1, ncols=3, figsize=(36, 10))
    ax = axes[0]

    age_label = "Age (CE)" if CE else "Age (BP)"

    plot_uncertainty_boxes(
        data_age, data_rsl, data_age_sigma * 2, data_rsl_sigma * 2, ax=ax, CE=CE
    )
...
def plot_spatial_rsl_single(pred_matrix, y_mean, y_var, cmap="viridis", time_unit="CE"):
    """
    ...
    unit_label = "CE" if str(time_unit).upper() == "CE" else "BP"
    ...
    cax = ax2.pcolor(
        lon_mat,
        lat_mat,
        y_mean.detach().numpy().reshape(lon_mat.shape),
    ...
    ax2.set_title("{:5.1f} {}".format(pred_matrix[0, 0], unit_label))
...
def plot_spatial_rsl_range(
    ...
    for i in range(len(time_mat)):
        mean_rsl += (
            y_mean[i :: len(time_mat)]
            .reshape([len(lat_matrix), len(lon_matrix)])
            .detach()
            .numpy()
        )
    ...
    ax2.set_extent(
        [np.min(lon_matrix), np.max(lon_matrix), np.min(lat_matrix), np.max(lat_matrix)]
    )

    cax = ax2.pcolor(
        lon_mat, lat_mat, mean_rsl, transform=ccrs.PlateCarree(), cmap=cmap
    )

    cbar = fig.colorbar(cax, ax=ax2, orientation="vertical", pad=0.01)

    cbar.set_label("RSL (m)")
    ax2.set_title("{:5.1f} to {:5.1f} {}".format(min_time, max_time, unit_label))

    # -----------------plot the RSL rate map-----------------
    rsl_rate = (
        y_mean[0 :: len(time_mat)] - y_mean[len(time_mat) - 1 :: len(time_mat)]
    ).detach().numpy().reshape([len(lat_matrix), len(lon_matrix)]) / (
        time_mat[0] - time_mat[-1]
    )
    ...
    if plot_site:
        for i in np.unique(rsl_region):
            ...
            ax2.scatter(
                np.mean(rsl_lon[time_index][region_index]),
    ...
    ax2.set_title("{:5.1f} to {:5.3f} {}".format(min_time, max_time, unit_label))
...
def plot_track_list(track_list):
    ...
    if row_num == 1:
        for i in range(row_num * 3):
            ...
            axes[i].set_title(
                "{} : {:6.6f}".format(track_list.columns[i], track_list.iloc[-1, i])
            )
        ...
        for i in range(row_num):
            for j in range(3):
                if i * 3 + j < track_list.shape[1]:
                    ...
                    axes[i, j].plot(
                        np.arange(len(track_list)), track_list.iloc[:, i * 3 + j].values
                    )

                    axes[i, j].set_title(
                        "{}: {:6.6f}".format(
                            track_list.columns[i * 3 + j],
