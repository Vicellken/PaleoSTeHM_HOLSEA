...
def check_geo_dim(X):
    ...
    if (X.dim() != 2) or (X.shape[1] != 3):
        raise ValueError(
            "The dimension of input X is not correct. If you use a spatio kernel, X should be in shape of n x 3: [age, lat, lon]."
        )
...
class Isotropy(Kernel):
    ...
    def __init__(
        ...
        if geo == False:
            ...
            s_lengthscale = (
                torch.tensor(1.0) if s_lengthscale is None else s_lengthscale
            )
    ...
    def _scaled_geo_dist2(self, X, Z=None):
        ...
        if Z is None:
            Z = X
        if X.dim() == 2:
            X = X[:, 1:]  # use lat and lon to calculate spatial distance
        if Z.dim() == 2:
            Z = Z[:, 1:]  # use lat and lon to calculate spatial distance
        ...
        a = (
            torch.sin(dlat / 2) ** 2
            + torch.cos(X_latitudes_rad[:, None])
            * torch.cos(Z_latitudes_rad[None, :])
            * torch.sin(dlon / 2) ** 2
        )
...
class RBF(Isotropy):
    ...
    def __init__(
        ...
        super().__init__(
            input_dim, variance, lengthscale, s_lengthscale, active_dims, geo
        )
...
class WhiteNoise_SP(Isotropy):
    ...
    def __init__(
        ...
        super().__init__(
            input_dim, variance, lengthscale, s_lengthscale, active_dims, geo, sp
        )
    ...
    def forward(self, X, Z=None, diag=False):
        ...
        if self.geo == True:
            ...
            # no correlation for points with longtitude larger than 360, which suppose to be psuedo data
            dis_fun = torch.outer(
                torch.tensor(X)[:, 1].abs() < 361, torch.tensor(Z)[:, 1].abs() < 361
            ).double()

            return self.variance * delta_fun.double() * dis_fun

        if self.geo == False:
            delta_fun = self._scaled_dist(X, Z) < 1e-4 & torch.outer(
                X[:, 2] < 361, Z[:, 2] < 361
            )
...
class DotProduct(Kernel):
    ...
    def __init__(self, input_dim, ref_year=None, variance=None, active_dims=None):
        ...
        variance = torch.tensor(1.0) if variance is None else variance
        self.variance = PyroParam(variance, constraints.positive)
        ref_year = torch.tensor(0.0) if ref_year is None else ref_year
...
class Linear(DotProduct):
    ...
    def forward(self, X, Z=None, diag=False):
        ...
        if Z.dim() > 1:
            Z = Z[:, 0]
        return self.variance * self._dot_product(
            X - self.ref_year, Z - self.ref_year, diag
        )
...
class Sampling(Kernel):
    ...
    def __init__(
        self, input_dim, model_ensemble=None, weights=None, active_dims=None, tau=None
    ):
        super().__init__(input_dim, active_dims)
    ...
    def forward(self, X, Z=None, diag=False):
        ...
        if not diag:
            if X is Z:
                cov_matrix = np.cov(
                    all_pred.detach().numpy(), aweights=self.weights, rowvar=False
                )
            else:
                all_pred_Z = torch.stack(
                    [torch.tensor(model(Z)) for model in self.ensemble]
                )
                cov_matrix = np.cov(
                    np.hstack([all_pred.detach().numpy(), all_pred_Z.detach().numpy()]),
            ...
            return (
                cov_matrix * taper_matrix.diag() if self.tau is not None else cov_matrix
            )
