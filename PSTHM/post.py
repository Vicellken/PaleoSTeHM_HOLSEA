...
def gen_pred_matrix(age, lat, lon):
    ...
    output_matrix = torch.tensor(
        np.hstack(
            [
                age_matrix.flatten()[:, None],
                lat_matrix.flatten()[:, None],
                lon_matrix.flatten()[:, None],
            ]
        )
    ).double()
...
def cal_rate_var(test_X, cov_matrix, mean_rsl, difftimestep=200):
    ...
    Mdiff = np.array(np.equal.outer(test_X, test_X.T), dtype=int) - np.array(
        np.equal.outer(test_X, test_X.T + difftimestep), dtype=int
    )
...
def cal_spatiotemporal_rate_var(test_X, cov_matrix, mean_rsl, difftimestep=200):
    ...
    if len(test_X.shape) == 1:
        ...
        Mdiff = np.array(
            np.equal.outer(test_X[:, 0], test_X[:, 0].T), dtype=int
        ) - np.array(
            np.equal.outer(test_X[:, 0], test_X[:, 0].T + difftimestep), dtype=int
        )
...
def get_change_point_posterior(guide, sample_number):
    ...
    for i in range(sample_number):
        ...
        output_dict["a"][i] = np.array(
            [posterior_samples[a_keys[j]].detach().numpy() for j in range(num_as)]
        )
        output_dict["cp"][i] = np.array(
            [posterior_samples[cp_keys[j]].detach().numpy() for j in cp_index]
        )
...
def change_point_forward(n_cp, cp_loc_list, new_X, data_X, beta_coef_list, b):
    ...
    """
    last_intercept = b
    cp_loc_prior = torch.linspace(data_X[:, 0].min(), data_X[:, 0].max(), n_cp + 3)[
        1:-1
    ]
    ...
    for i in range(n_cp + 1):
        ...
        mean[start_idx:end_idx] = (
            beta_coef_list[i] * (new_X[start_idx:end_idx:, 0] - last_change_point)
            + last_intercept
        )
        last_intercept = (
            beta_coef_list[i] * (end_age - last_change_point) + last_intercept
        )
